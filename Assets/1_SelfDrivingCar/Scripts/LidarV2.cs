...
public class LidarV2 : MonoBehaviour
{
	...
	void Update()
	{
        ...
        //var data = lastImage.GetRawTextureData();


        //test************************************************

        //Color[] data = lastImage.GetPixels();

        Color pixel_i = lastImage.GetPixel(900, 125);   //was 114

       
       // int r = (1809 * 114) + 904;


        //Debug.Log("r=" + data[r].r + ",  x=" + (130 * data[r].r)); // + ( Mathf.Exp(-15 * data[r].r)));
        Debug.Log("r=" + pixel_i.r + ",  x=" + (140 * pixel_i.r) );
